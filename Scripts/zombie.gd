...
func _physics_process(delta: float) -> void:
	if hero.global_position.distance_to(officer.global_position)>=100 and hero.global_position.distance_to(officer.global_position)<2000 and State != 'death':
...
func _on_zombia_area_area_entered(area: Area2D) -> void:
