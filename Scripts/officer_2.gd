...
func _physics_process(delta: float) -> void:
	if hero.global_position.distance_to(officer.global_position)>=500 and hero.global_position.distance_to(officer.global_position)<2000 and State != 'death':
...
func shoot():
	if State == "Shoot":
		look_at(hero.global_position)
		if  hero.global_position.distance_to(officer.global_position) < 300:
		...
		if aiming:
			...
			#bullet.speed = 3000
			#bullet.dir = rotation
			#bullet.set_bullet_type("officer2")
...
func Robowalk():
	if State == 'Running':
		$AnimatedSprite2Dflames.visible = false
		$flamesArea/CollisionShape2Dflames.disabled = true
		$AnimatedSprite2Dflames.stop()
		officer.play("Run")
